...
def init_device_widgets(device_id, device_type, base_widgets):
    """Initialize widget layout for a new device based on type."""
    scale = 1.0 if device_type == 'laptop' else 0.7
    conn = get_db()
    for bw in base_widgets:
        w = max(2, round(bw['w'] * scale))
        h = max(2, round(bw['h'] * scale))
        extra_keys = {k: v for k, v in bw.items()
                      if k not in ('id', 'x', 'y', 'w', 'h', 'css', 'content', 'lock_ratio')}
        conn.execute("""
            INSERT OR REPLACE INTO device_widgets
                (device_id, widget_id, x, y, w, h, css, extra)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (device_id, bw['id'], bw['x'], bw['y'], w, h,
              bw.get('css', ''), json.dumps(extra_keys)))
    conn.commit()
    conn.close()
